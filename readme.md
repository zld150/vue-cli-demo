# 说明文档
## `zld`: 一个帮助你快速搭建和开发前端项目的CLI，参照coderwhy老师！
...
````shell
zld addcpn YourComponentName # 例如zld add NavBar，默认会存放到src/components文件夹中
zld addcpn YourComponentName -d src/pages/home # 也可以指定存放的具体文件夹
````
...
```shell
zld addpage YourPageName # 例如zld addpage Home，默认会放到src/pages/home/Home.vue中，并且会创建src/page/home/router.js
zld addpage YourPageName -d src/views # 也可以指定文件夹，但需要手动集成路由
```
...
```shell
zld addstore YourVuexChildModuleName # 例如zld addstore home，默认会放到src/store/modules/home/index.js和types.js
zld addstore YourVuexChildModuleName -d src/vuex/modules # 也可以指定文件夹
```
...
## `zld`: A CLI to help you quickly build and develop front-end projects

> Can't think of other names, so just use zld~
...
```shell
addcpn YourComponentName zld # example zld add NavBar, the default will be stored in src / components folder 
zld addcpn YourComponentName -d src / Pages and the / Home # specific file can also specify the destination folder
```

### Create a Vue page and configure routing

```shell
addPage YourPageName zld # example zld addpage Home, the default will put src / pages / home / Home.vue, and will create a src / Page / Home / router.js 
zld addPage YourPageName -d src / views # You can also specify a folder , But need to integrate routing manually
```
...
```shell
addstore YourVuexChildModuleName zld # example zld addstore home, the default will put src / store / modules / home / index.js and types.js 
zld addstore YourVuexChildModuleName -d src / vuex / modules # You can also specify a folder
```
